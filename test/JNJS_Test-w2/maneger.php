...
<body>
  ...
  <main>
    ...
    <div id="user-data">
      <div id="srch-box">
        <input type="search" name="srch-txt" id="srch-txt" value="<?php if(!empty($_GET["srch-txt"])) echo $_GET["srch-txt"]; ?>">
        <button id="srch-btn" class="ui-icon ui-icon-search zoom" style="border: none; cursor: pointer;"></button>
      </div>
      <table border="1">
        <thead>
          <?php if($permission[0] >=2) echo "<th>修改</th>" ?>
          <?php if($permission[0] >=2) echo "<th>刪除</th>" ?>
            ...
            while($row = $result->fetch_array()) {
              ...
              if($permission[0] >= 2) {
                if($row["permission"] == 3){
                  ...
                  echo 
                  "<td class=\"edit-user-btn\"><span class=\"ui-icon ui-icon-pencil zoom\"></span></td>
                  <td class=\"del-user-btn\"><span class=\"ui-icon ui-icon-circle-minus zoom\"></span></td>";
                }
              }
              echo
                "<td>".$row['id']."</td>
